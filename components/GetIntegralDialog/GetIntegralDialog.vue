  ...
  >
    ...
    <div class="get-integral-content-box">
      ...
      <div class="current-total">
        （当前简币数量：{{ useUserInfoStore().userIntegralInfo.integralTotal }}
      ...
      <div v-config:open_sign_in class="way way-1">
        ...
        <div class="attendance-box">
          <div
            v-if="!useUserInfoStore().userIntegralInfo.isattendance"
            class="button"
            @click="toAttendance"
          >
            签到
      ...
      <div class="way way-2">
        <p class="way-tips"
          ...
          <span class="small-tips"
            >（每日最多+{{ config.public.mostIntegralComment }}，评论被删除将扣除已获得简币！）</span
          >
        </p>
      </div>
...
import { useUserInfoStore } from '~/store/user';
  ...
  const toAttendance = async () => {
    ...
    if (data.data.status === 200) {
      ...
      // 更新用户简币信息
      useUserInfoStore().getUserIntegralTotal();
    } else {
