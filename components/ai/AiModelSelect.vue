<template>
      ...
      <el-radio-group v-model="selectedModel" @change="handleModelChange">
        ...
        <template v-if="modelList.length > 0">
          <el-tooltip v-for="(item, index) in modelList" :key="index" effect="dark"
            :content="`每次消耗 ${Math.abs(payValue)} 简币`" placement="top">
  ...
  <!-- 获取简币弹窗 -->
  <pay-integral-dialog :title="title" :dialog-get-integral-visible="dialogGetIntegralVisible"
    :pay-number="-Math.abs(payValue) || 0" placeholder="获取简币" @cancle="handleCancleDialog"
