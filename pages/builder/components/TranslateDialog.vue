<!-- ai帮写弹窗 -->
<template>
  <el-dialog :model-value="dialogTranslateVisible" title="请选择需要转换成的语种" width="420px" :show-close="false"
    :close-on-click-modal="false" append-to-body>
    <div v-loading="aiLoading" class="ai-content-translate-select-warpper" element-loading-text="AI拼命翻译中~">
      <p class="tips"><b>注意：</b>此操作AI会将您的简历内容翻译为对应语种，无法确保完全正确，请你最好留有数据备份！</p>
      <el-form size="normal">
...
import { translateTextAsync, cancleAiTranslateTextAsync } from '~/composables/api/ai';
...
watch(
  ...
  async (newVal) => {
    if (newVal) {
      ...
      const { HJNewJsonStore } = useCreateTemplateStore();
...
const submit = async () => {
  ...
  aiEditContent.value = data.content.Text;
  try {
