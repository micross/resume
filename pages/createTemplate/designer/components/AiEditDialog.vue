<template>
  <el-dialog :model-value="dialogAiVisible" :title="dialogTitle" width="800px" :show-close="false"
    :close-on-click-modal="false" append-to-body custom-class="custom-dialog" @open="handleOpen">
    <div class="ai-content-edit-page-select-warpper">
      <!-- 当前简币数量 -->
      <div class="content-box">
        <h1 class="title">您当前简币数量
      ...
      <div class="model-selector">
        <el-radio-group v-model="selectedModel">
          ...
          <template v-if="modelList.length > 0">
            <el-tooltip v-for="(item, index) in modelList" :key="index" effect="dark"
              :content="`每次消耗 ${item.cost} 简币`" placement="top">
      ...
      <div class="bottom">
        <div class="ai-content ai-content-left">
          ...
          <el-input v-else v-model="aiPropmt" type="textarea" :placeholder="placeholder" maxlength="3000"
            show-word-limit>
          </el-input>
        </div>
        <div class="ai-content-center">
          <svg-icon :class="['ai-icon', { 'ai-icon-loading': aiLoading }]" icon-name="icon-multimediaaiduomeitiai"
            color="#4e97fb" size="40px" @click="aiEdit" />
          <template v-if="type === 'edit'">
        ...
        <div class="ai-content ai-content-right">
          ...
          <el-input v-model="aiEditContent" type="textarea"
            :element-loading-text="type === 'edit' ? 'AI拼命润色中~' : '拼命创作中'" placeholder="AI创作内容将展示在这里">
          </el-input>
        </div>
      </div>
    </div>
  ...
  <!-- 获取简币弹窗 -->
  <PayIntegralDialog :title="title" :dialog-get-integral-visible="dialogGetIntegralVisible"
    :pay-number="-Math.abs(payValue) || 0" placeholder="下载该创作" @cancle="handleCancleDialog"
    @confirm="handleConfirmDialog"></PayIntegralDialog>
</template>
...
const placeholder = ref<string>('请输入关键词，如毕业于红黄简历大学、计算机专业、创作项目经历等');
...
const aiEdit = async () => {
  ...
  const params = {
    model: defaultModel.value.length > 0 ? defaultModel.value : null,
    text: props.type === 'edit' ? props.content : aiPropmt.value,
    module_title: props.module.title
  };
    ...
    () => {
      ...
      if (defaultModel.value) {
        ...
        useUserInfoStore().userIntegralInfo.integralTotal =
          useUserInfoStore().userIntegralInfo.integralTotal + payValue.value;
      }
