<template>
  <el-dialog :model-value="dialogGenerateVisible" title="AI智能生成简历" width="700px" :show-close="false"
    :close-on-click-modal="false" append-to-body @open="handleOpen">
    <div class="ai-generate-dialog-content">
      <!-- 关键词填写 -->
      <KeyWords v-show="active === 0 && !isAiLoading" ref="keyWordsRef"></KeyWords>
      <!-- 模型选择 -->
      <AiModelSelect v-show="active === 1 && !isAiLoading" ref="aiModelSelectRef"></AiModelSelect>
      <!-- AI生成中 -->
    ...
    <template #footer>
      <span class="dialog-footer">
        ...
        <el-button v-if="active === 1 && !isAiLoading" type="primary" @click="prevStep">上一步</el-button>
...
import KeyWords from '@/pages/generateAiResume/components/KeyWords.vue';
import AiModelSelect from '@/pages/generateAiResume/components/AiModelSelect.vue';
...
import { cancelGenerateResumeStreamAsync, generateResumeStreamAsync } from '~/composables/api/ai';
...
const { HJNewJsonStore, fromAiGenerate } = storeToRefs(useCreateTemplateStore());
const handleOpen = () => {
  active.value = 0;
