<template>
  <el-drawer :model-value="drawer" :size="820" class="ai-optimize-drawer" direction="rtl" title="AI诊断结果"
    ...
    <div ref="contentRef" v-loading="isLoading" v-dompurify-html="renderedContent"
      class="ai-optimize-content markdown-body"></div>
  </el-drawer>
</template>

<script lang="ts" setup>
import { cancelOptimizeResumeStreamAsync, optimizeResumeStreamAsync } from '~/composables/api/ai';
...
const formattedIntegral = computed(() => {
  return formatNumberWithCommas(useUserInfoStore().userIntegralInfo.integralTotal);
});
    ...
    () => {
      if (props.modelInfoObj.selectedModel) {
        ...
        useUserInfoStore().userIntegralInfo.integralTotal =
          useUserInfoStore().userIntegralInfo.integralTotal + props.modelInfoObj.payValue;
      }
      isLoading.value = false;
