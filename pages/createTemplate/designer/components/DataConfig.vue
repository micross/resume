    ...
    <div class="data-config-title-box">
      <div class="left">
        <el-popover placement="bottom" :width="400" trigger="click">
          ...
          <div class="module-fast--popver-content-box">
            <div v-for="(moduleItem, index) in HJNewJsonStore.componentsTree" :key="moduleItem.id" class="module-item">
              <div class="left-title">{{ moduleItem.title }}</div>
              <data-title-right :id="moduleItem.id" :module="moduleItem"
                :is-unfold="moduleItem.customProps.unfoldModule" :is-show-module="moduleItem.customProps.showModule"
                @open-style-drawer="handleOpenStyleDrawer" @handle-unfold="handleUnfold(index)"
                @handle-collapse="handleCollapse(index)"
                @handle-change-switch="handleChangeSwitch($event, index)"></data-title-right>
            </div>
          </div>
        </el-popover>
        <el-popover v-if="route.query.type !== 'create'" placement="bottom" :width="400" trigger="click">
          <template #reference>
            <div>
              <el-tooltip effect="light" content="快速添加模块" placement="bottom">
                <svg-icon icon-name="icon-zengjiatianjiajiahao" color="#74a274" size="28px"></svg-icon>
              </el-tooltip>
            </div>
          </template>
          <!-- 模块列表选择 -->
          <div class="module-selected--popver-content-box">
            <div v-for="(value, key) in userAddModule" :key="key" :class="[
              'add-module-item',
              { existence: existenceModule.indexOf(value.category) > -1 }
            ]" @click="handleAddModule(value)">
              <el-tooltip effect="light" content="已有该模块" placement="bottom"
                :disabled="existenceModule.indexOf(value.category) < 0">
                <div>
                  <svg-icon :icon-name="value.icon"
                    :color="existenceModule.indexOf(value.category) > -1 ? '#ccc' : '#606266'" size="16px"></svg-icon>
      ...
      <div class="right">
        ...
        <div class="unfold-box-all" @click="openAll">
          ...
          <svg-icon :class="['open-all-icon', { 'open-all': isOpenAll }]" icon-name="icon-z044" color="#74a274"
            size="13px" />
        </div>
      </div>
    </div>

    <ElScrollbar trigger="always" style="height: 100%; background-color: #fff">
      <template v-for="(moduleItem, index) in HJNewJsonStore.componentsTree" :key="moduleItem.id">
        <div :ref="(el) => getDataModuleRef(el, moduleItem)"
          ...
          <div :class="[
            ...
            <DataTitleLeft :id="moduleItem.id"></DataTitleLeft>
            <DataTitleRight :id="moduleItem.id" :module="moduleItem" :is-unfold="moduleItem.customProps.unfoldModule"
              :is-show-module="moduleItem.customProps.showModule" @open-style-drawer="handleOpenStyleDrawer"
              @handle-unfold="handleUnfold(index)" @handle-collapse="handleCollapse(index)"
              @handle-change-switch="handleChangeSwitch($event, index)"></DataTitleRight>
          </div>
              ...
              <template v-for="(value, key, dataIndex) in moduleItem.dataSource" :key="dataIndex">
                ...
                <template v-else>
                  <div v-if="moduleItem.props[key]?.config" :style="getFiledStyle(value)">
                    <component :is="dataSourceCptMap[value.type]" v-model="moduleItem.dataSource[key].value"
                      :label="moduleItem.dataSource[key].label" :key-value="key" :module="moduleItem"
                      :disabled="!moduleItem.props[key].config" :module-id="moduleItem.id">
                      <!-- 组件图标 -->
                      <template #label-left>
                        <icon-select-pop v-if="moduleItem.props[key].iconfont" v-model="moduleItem.props[key].iconfont"
                          size="18px"></icon-select-pop>
                      </template>
                      <!-- 组件类型切换 -->
                      <template #component-switch>
                        <ComponentTypePop v-model="moduleItem.dataSource[key].type"
                          :content="moduleItem.dataSource[key]" :type="value.type"
                          @editor-switch="handleEditorSwitch($event, moduleItem.dataSource[key])"></ComponentTypePop>
                      </template>
                      <!-- 组件开关 -->
                      <template #label-right>
                        <div class="field-label-right-box">
                          <el-switch v-model="moduleItem.props[key].show" :disabled="!moduleItem.props[key].config" />
                        </div>
                      </template>
                    </component>
                  </div>
      ...
      <!-- 模块样式设置弹窗 -->
      <ModuleStyleSettingDrawer :drawer="styleDrawer"
        @close-style-drawer="handleCloseDrawer"></ModuleStyleSettingDrawer>
    </ElScrollbar>
...
const { dataConfigScrollToView } = useCreateTemplateStore();
// 打开模块样式设置抽屉
...
// 选中模块
const handleSelectDataModule = (moduleItem: ResumeComponent) => {
