<template>
  <div class="data-title-right-box">
    <!-- 删除图标 -->
    <el-popconfirm confirm-button-text="确定" cancel-button-text="取消" width="220" :icon="InfoFilled" icon-color="#626AEF"
      title="确定删除模块？数据将不会保留！" @confirm="handleDeleteIcon">
      <template #reference>
        <div>
          <el-tooltip effect="light" :content="isCanDelete ? '删除模块' : '无法删除最后一个模块'" placement="bottom">
            <svg-icon :class="['delete-icon', { 'not-delete': !isCanDelete }]" icon-name="icon-shanchu5" color="#1e2532"
              size="24px" @click="handleClickDelete" />
          </el-tooltip>
        </div>
      </template>
    </el-popconfirm>
    ...
    <el-tooltip effect="light" content="样式设置" placement="bottom">
      <svg-icon icon-name="icon-tesezhuti" color="#1e2532" size="24px" @click.stop="handleStyleSetting" />
    </el-tooltip>
    <!-- 显示或隐藏模块 -->
    <el-tooltip effect="light" :content="isShowModule ? '隐藏' : '显示'" placement="bottom">
      <svg-icon v-if="isShowModule" icon-name="icon-jurassic_openeyes" color="#1e2532" size="26px"
        @click.stop="handleChangeSwitch(false)" />
      <svg-icon v-else icon-name="icon-closeEye" color="#1e2532" size="26px" @click.stop="handleChangeSwitch(true)" />
    </el-tooltip>
    <!-- 展开收起 -->
    <el-tooltip effect="light" :content="isUnfold ? '收起' : '展开'" placement="bottom">
      <svg-icon :class="['unfold', { collapse: !isUnfold }]" icon-name="icon-shangyi" color="#1e2532" size="24px"
...
const { selectedModuleId, HJNewJsonStore } = storeToRefs(useCreateTemplateStore());
