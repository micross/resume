...
<template>
  <div class="module-select-list">
    <div class="module-select">
      ...
      <ElScrollbar trigger="hover">
        <ul>
          <li v-for="(val, index) in modulesList" :key="index" :class="[{ active: selectedModuleListId === val.id }]"
    ...
    <div class="module-list-box">
      ...
      <template v-if="selectedModuleListId">
        <!-- 模块列表图 -->
        <ElScrollbar v-if="currentModuleListObj.list.length" trigger="hover">
          <div v-viewer class="list-box">
            <!-- 拖拽组件 -->
            <draggable class="dragArea list-group" :sort="false" :list="currentModuleListObj.list" :clone="cloneData"
              chosen-class="chosen" :group="{ name: 'custom', pull: 'clone', put: false }" item-key="id" @start="start">
              <template #item="{ element }">
                <div class="img-box" :style="getImgBoxStyle(element?.screenShot)">
                  <img :src="getImgListStyleImageFile(
...
const { selectedModuleListId } = storeToRefs(useCreateTemplateStore());
...
// 拖拽时，初始化数据
const cloneData = (data: ResumeComponent) => {
...
// 组件数据
const handleData = (data: ResumeComponent) => {
