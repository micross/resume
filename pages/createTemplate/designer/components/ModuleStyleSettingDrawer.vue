<template>
  <el-drawer :model-value="drawer" :modal="true" :size="700" class="module-style-setting-drawer"
    modal-class="modal-style-setting-drawer" :direction="route.query.type !== 'create' ? 'ltr' : 'rtl'" title="模块样式设置"
    destroy-on-close @close="handleClose" @open="handleOpen">
    <el-collapse v-model="activeNames">
      ...
      <el-collapse-item title="组件整体样式" name="styleProp">
        <el-form label-width="100px" label-position="right">
          <div v-for="(value, key, index) in module.css" :key="index">
            <component :is="getStyleSetterCom(key)" :id="selectedModuleId" :value="value"></component>
          </div>
        </el-form>
      </el-collapse-item>
      <!-- 自定义的样式属性 -->
      ...
      <el-collapse-item v-for="(customItem, customIndex) in module.customCss" :key="customIndex"
        :title="customItem.title" :name="customItem.prop">
        <el-form label-width="100px" label-position="right">
          ...
          <div v-for="(value, key, index) in customItem.css" :key="index">
            <component :is="getStyleSetterCom(key)" :id="selectedModuleId" :custom-css-prop="customItem.prop"
...
// 打开弹窗
const module = ref<ResumeComponent | any>({});
const handleOpen = () => {
