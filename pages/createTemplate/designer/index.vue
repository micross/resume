<template>
  <div class="create-template-box">
    <!-- 导航栏 -->
    <NavBar :template-info="templateInfo" @publish-success="publishSuccess" @refresh="generateSuccess"></NavBar>
    <!-- 内容区容器 -->
...
// 重置
const { resetKey, fromAiGenerate } = storeToRefs(useCreateTemplateStore());
...
// 初始化JSON数据
const { HJNewJsonStore, selectedPageName } = storeToRefs(useCreateTemplateStore());

// 查询模版数据
const getTemplateData = async () => {
  const { template } = useTemplateDetail(id.value);
