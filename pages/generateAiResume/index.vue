...
<script lang="ts" setup>
  ...
  import { getUsertemplateAsync } from '~/composables/api/createTemplate';
...
import { useTemplateDetail } from '~/composables/templates';
  ...
  const getTemplateData = async () => {
    const { template } = useTemplateDetail(selectTemplateId.value);
    ...
    try {
      ...
      if (data.status === 200) {
        ...
        try {
          ...
          router.push({
            path: `/builder/${selectTemplateId.value}`
          });
        } catch (error) {
          // 用户点击了"取消"或关闭了对话框
