<template>
  <div class="membership-box">
    <!-- 背景图 -->
    <div class="bg-box">
      <img src="/images/membership/member-bgc.png" />
    </div>
    <!-- 内容区 -->
    <div class="membership-content-box">
      ...
      <div class="pay-content-box">
        <div v-if="!loading" class="member-goods-container">
          <div v-for="(item, index) in memberTypes" :key="index" :class="[
            'member-goods-item',
            item.member_type,
            { active: selectedItem?.id === item.id }
          ]" @click="handleMembership(item)" :data-item_id="item.id">
            <template>
              <img v-show="selectedItem?.id === item.id" :src="`/images/membership/${item.member_type}.gif`" />
            </template>
            <div class="info-container">
              <div :class="['goods-name', `name-${item.name}`]">{{ item.name }}</div>
              <div :class="['goods-price', `name-${item.name}`]">
                <span class="rmb-icon"> ¥ </span>
                <span class="price-num">{{ item.price }}</span>
              </div>
              <div :class="['price-desc', `desc-${item.name}`]">
                原价<span class="price-line">{{ item.original_price }}￥</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="bottom-pay-wrapper">
          <div class="pay-info">
            ...
            <span data-v-0be21c4c="" class="pay-info-num">{{ selectedItem?.price }}</span>
          </div>
          <!-- 充值方式 -->
          <div class="pay-way">
            <h3>支付方式</h3>
            <div class="pay-way-box">
              <div :class="['pay-way-zfb', { 'pay-active': payType === 'zfb' }]" @click="selectPayWay('zfb')">
                <img src="@/assets/images/pay/zfb.png" alt="支付宝" />
              </div>
              <div :class="['pay-way-wx', { 'pay-active': payType === 'wxpay' }]" @click="selectPayWay('wxpay')">
          ...
          <div class="pay-btn" @click="getOrderQrcode">
            ...
            <div class="pay-btn-light">
              <img src="/images/membership/line.png" />
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="member-benefit">
        ...
        <div class="vip-yige">
          <div class="welfareBg">
            <div v-for="(color, index) in colors" :key="index" class="lineBg" :class="'color' + (index % 2)"></div>
          </div>
          <div class="cotent-item-wrapper first">
            <div class="cotent-item cotent-item-type">
              ...
              <div class="welfare first-column">
                <div v-for="(benefit, index) in benefits" :key="index" class="line" :class="{ firstLine: index === 0 }">
                  {{
                    benefit.name }}</div>
              </div>
            </div>
          </div>
          <div v-for="(membership, index) in memberships" :key="index" class="cotent-item-wrapper"
            ...
            <div class="cotent-item" :class="{ active: membership.isActive }">
              ...
              <div class="welfare">
                <div v-for="(benefit, bIndex) in membership.benefits" :key="bIndex" v-dompurify-html="benefit.content"
                  class="line" :class="{
    ...
    <!-- 充值弹窗 -->
    <buy-qr-code-dialog :model-value="dialogQrcodeVisible" :order-type="2" @pay-success="handlePaySuccess"
      :member-type="selectedItem" @cancel="handleCancel"></buy-qr-code-dialog>

    <!-- 微信充值二维码弹窗 -->
    <w-x-buy-qr-code-dialog :model-value="dialogWXQrcodeVisible" :order-type="2" pay-type="wxpay"
      :member-type="selectedItem" @pay-success="handleWXPaySuccess" @cancel="handleWXCancel"></w-x-buy-qr-code-dialog>
  </div>
</template>
...
import { useMemberTypes } from '~/composables/member';
...
// 获取会员配置列表
const { memberTypes, loading } = useMemberTypes();


const selectedItem = ref<MemberType | null>(null);
  ...
  .membership-content-box {
        ...
        .membership-time {
          ...
          .user-info-box {
            ...
            .not-membership-img {
              ...
              cursor: pointer;
              background: url(/images/membership/not-membership.png) 100% / cover no-repeat;
    ...
    .pay-content-box {
      ...
      .member-goods-container {
        ...
        .monthly {
          background-image: url(/images/membership/membership-monthly.png);
        }

        .yearly {
          background-image: url(/images/membership/membership-yearly.png);
        }

        .lifetime {
          background-image: url(/images/membership/membership-lifetime.png);
        }
      }
