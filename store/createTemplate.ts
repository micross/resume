...
import { HJNewSchema } from '@/pages/createTemplate/designer/schema/template';
import type { ResumeComponent, TemplateJson } from '~/lib';
...
export const useCreateTemplateStore = defineStore('createTemplate', () => {
  ...
  const HJNewJsonStore = ref<TemplateJson>(resume_json);
  const resetKey = ref<number>(0);
  const fromAiGenerate = ref<boolean>(false); // 是否来源于AI生成的简历
  function changeResumeJsonData(obj: TemplateJson) {
    HJNewJsonStore.value = cloneDeep(obj);
  }
  function pushComponent(data: ResumeComponent) {
