...
export const formatDate = (dataArray: Array<string> | string): string => {
  if (Array.isArray(dataArray)) {
    const startDate = dayjs(new Date(dataArray[0])).format('YYYY.MM').split('-').join('.');
    let endDate;
    if (dataArray[1] === '至今') {
      endDate = '至今';
    } else {
      endDate = dayjs(new Date(dataArray[1])).format('YYYY.MM').split('-').join('.');
    }
    return `${startDate}-${endDate}`;
  } else {
    return dayjs(new Date(dataArray)).format('YYYY.MM').split('-').join('.');
  }
};
...
export const formatDateToYMD = (dateString: string): string => {
  if (dateString) {
    return dayjs(new Date(dateString)).format('YYYY 年 MM 月 DD 日');
  } else {
    return '暂无数据';
  }
};
...
export const checkVersion = () => {
  ...
  if (!version) {
    useState('resumeDraft').value = ''; // 移出本地简历数据
  } else {
    if (Number(version) !== 6) {
      useState('resumeDraft').value = ''; // 移出本地简历数据
    }
  }
  useState('version').value = '6'; // 存入版本
};
